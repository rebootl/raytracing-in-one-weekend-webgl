...
const drawParticlesVS = `
  ...
  vec4 getValueFrom2DTextureAs1DArray(sampler2D tex,
    vec2 dimensions, float index) {
  ...
  void main() {
    ...
    vec4 position = getValueFrom2DTextureAs1DArray(positionTex,
      texDimensions, id);
...
const drawParticlesFS = `
  precision highp float;
  void main() {
    gl_FragColor = vec4(0.2, 0.4, 0.7, 1);
  }
`;
...
/*const updateVelocityProgram = webglUtils.createProgramFromSources(
    gl, [updatePositionVS, updateVelocityFS]);*/
...
// setup an id buffer
const particleTexWidth = 200;
const particleTexHeight = 200;
...
//const velocityTex2 = createTexture(gl, velocities, particleTexWidth, particleTexHeight);
...
const velocitiesFB1 = createFramebuffer(gl, velocityTex);
